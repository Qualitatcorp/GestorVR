 ...
 *
 * The followings are the available columns in table 'rv_int_tipo':
 ...
 * @property RvTipo $tev
 */
class RvIntTipo extends CActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			array('tev_id, language', 'required'),
			array('tev_id', 'numerical', 'integerOnly'=>true),
			array('language', 'length', 'max'=>2),
			array('nombre, descripcion', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('int_id, tev_id, language, nombre, descripcion', 'safe', 'on'=>'search'),
		);
	}
	 ...
	 * @return RvIntTipo the static model class
	 */
